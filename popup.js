...
async function openAdminPanel() {
    // Проверяем права доступа (можно добавить пароль)
    const apiKey = await getApiKey();
    if (apiKey === 'sk-04f0f810450346fcb0c73748baa2fadf') { // Ваш валидный ключ
        chrome.tabs.create({ url: chrome.runtime.getURL('admin.html') });
    } else {
        showError('Доступ к админ-панели ограничен');
    }
}
...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    if (adminPasswordSubmit) {
        adminPasswordSubmit.addEventListener('click', () => {
            if (adminPasswordInput.value === 'P@ssw0rd') {
                adminPasswordModal.style.display = 'none';
                chrome.tabs.create({ url: chrome.runtime.getURL('admin.html') });
            } else {
...
function setupEventListeners() {
    // document.getElementById('adminBtn').addEventListener('click', openAdminPanel); // отключено, теперь только через пароль
...
async function analyzeCurrentPage() {
    try {
        ...
        appendLog('INTERACTION', 'Запрос анализ страницы', tab && tab.url ? tab.url : 'URL не найден');
...
async function analyzeTextHandler() {
    ...
    appendLog('INTERACTION', 'Запрос анализ текста', text);
...
async function performAnalysis(text, title = 'Анализ') {
    ...
    try {
        ...
        try {
            ...
            console.error('[Этап 1] Ошибка подготовки UI:', err);
        }

        // Этап 2: генерация ID
        try {
            currentAnalysisId = 'analysis_' + Date.now();
            console.log('[Этап 2] Запуск анализа с ID:', currentAnalysisId);
        } catch (err) {
            console.error('[Этап 2] Ошибка генерации ID:', err);
        }
        ...
        try {
            ...
            console.error('[Этап 3] Ошибка сохранения состояния:', err);
        }

        // Этап 4: установка обработчика сообщений
        try {
            setupMessageHandler();
        } catch (err) {
            console.error('[Этап 4] Ошибка установки обработчика:', err);
        }

        // Этап 5: отображение прогресса
        try {
            showProgress('Запуск анализа...', 10);
        } catch (err) {
            console.error('[Этап 5] Ошибка отображения прогресса:', err);
        }

        // Этап 6: отправка запроса на анализ
        try {
            chrome.runtime.sendMessage({
                ...
                if (chrome.runtime.lastError) {
                    console.error('[Этап 6] Ошибка отправки:', chrome.runtime.lastError);
                    handleAnalysisError('Не удалось отправить запрос на анализ: ' + chrome.runtime.lastError.message);
                    return;
                }

                console.log('[Этап 6] Ответ от background:', response);

                if (response && response.success) {
                    try {
                        showProgress('Анализ запущен...', 20);
                    } catch (err) {
                        console.error('[Этап 6] Ошибка отображения прогресса после запуска:', err);
                    }
            ...
            console.error('[Этап 6] Ошибка отправки запроса на анализ:', err);
            handleAnalysisError('Ошибка отправки запроса: ' + err.message);
        }

    } catch (error) {
        console.error('[performAnalysis] Глобальная ошибка:', error);
        ...
        setTimeout(() => {
            try {
                ...
                console.error('[finally] Ошибка восстановления кнопки:', err);
            }
        }, 1000);
    }
}
...
function displayAnalysisResult(result, customDiv) {
    ...
    if (result.sources_validation) {
        ...
        let score = result.sources_validation.cross_verification_score || 0;
        ...
        validationSection.innerHTML = `
            ...
            <div class="analysis-details">
                ...
                <strong>Оценка перекрестной проверки:</strong> ${score}/10
            </div>
        `;
...
function displayChatHistory() {
    ...
    if (chatHistory.length === 0) {
        ...
        welcomeMessage.innerHTML = 'Привет! Я ваш AI помощник. Чем могу помочь?';
        messagesDiv.appendChild(welcomeMessage);
        // Прокрутка к приветственному сообщению
        setTimeout(() => {
            messagesDiv.scrollTop = messagesDiv.scrollHeight;
        }, 100);
        return;
    ...
    // Гарантированная прокрутка вниз
    setTimeout(() => {
        messagesDiv.scrollTop = messagesDiv.scrollHeight;
    }, 100);
}
...
async function sendMessage() {
    ...
    if (!message) return;
    appendLog('INTERACTION', 'Отправка сообщения', message);
    ...
    // Прокрутка после добавления пользовательского сообщения
    const messagesDiv = document.getElementById('messages');
    messagesDiv.scrollTop = messagesDiv.scrollHeight;
    ...
    // Прокрутка после добавления сообщения "Думаю..."
    setTimeout(() => {
        messagesDiv.scrollTop = messagesDiv.scrollHeight;
    }, 100);
    ...
    if (!apiKey || apiKey !== validKey) {
        ...
        displayChatHistory();
        // Прокрутка после отображения ошибки
        setTimeout(() => {
            messagesDiv.scrollTop = messagesDiv.scrollHeight;
        }, 100);
...
async function resendPendingMessage(message) {
    try {
        ...
        const messages = [
            { role: 'system', content: 'Ты - полезный AI помощник. Отвечай точно и информативно.' },
        ...
        // Гарантированная прокрутка после получения ответа
        setTimeout(() => {
            messagesDiv.scrollTop = messagesDiv.scrollHeight;
        }, 100);
...
async function saveApiKey() {
    ...
    try {
        ...
        const analyzeTextBtn = document.getElementById('analyzeTextBtn');
        if (analyzePageBtn) analyzePageBtn.disabled = false;
